...
class CONTROLLER:
    ...
    rd = torch.zeros([2])
    rd_prev = torch.zeros([2])
    rd_dot_tao = torch.zeros([2])
    encoder1 = 0
    encoder3 = 0
                    ...
                    float(msg_ctrl.g_inverse[0]),float(msg_ctrl.g_inverse[1]),float(msg_ctrl.g_inverse[2]),float(msg_ctrl.g_inverse[3]),float(msg_ctrl.er[0]),float(msg_ctrl.er[1]), \
                        float(msg_ctrl.ur[0]),float(msg_ctrl.ur[1]),float(msg_ctrl.u[0]),float(msg_ctrl.u[1]),float(msg_ctrl.send_vel[0]),float(msg_ctrl.send_vel[1]),float(msg_ctrl.send_hub[0]),float(msg_ctrl.send_hub[1]),float(self.rd[0]),float(self.rd[1]),float(self.r[0]),float(self.r[1]),float(self.r_dot[0]),float(self.r_dot[1]),\
                            float(self.rd_ddot[0]),float(self.rd_ddot[1]),float(self.rd_dot[0]),float(self.rd_dot[1]),\
                                float(self.encoder1),float(self.encoder3),float(self.encoder_step1),float(self.encoder_step2),float(self.tao),float(self.U)))
    ...
    def suspension_pos_vel_field(self):
        """
        input:
        self.xd:2*1 tensor   desired position xd = xd(tao)
        self.x:2*1 tensor    real position 


        output: self.xd_dot:2*1 tensor desired velocity 
        """

        R = 6
        omega = 0.1
        #U = 1/2*self.beita_1*((self.r[0]-self.rd[0])**2 + (self.r[1]-self.rd[1])**2)
        U = 1/2*self.beita_1*(1-math.cos(self.r[0]-self.rd[0]) + 1-math.cos(self.r[1]-self.rd[1]))
        lambda_1 = math.exp(-self.beita_2*U)
        lambda_2 = 2.0-lambda_1 
        dtao = lambda_1*self.dt
        self.tao += dtao

        self.rd_prev = self.rd.clone()
        
        self.rd[0] = R*math.sin(omega*self.tao)
        self.rd[1] = -R+R*math.cos(omega*self.tao)

        #self.rd[0] = 0.9*self.tao
        #self.rd[1] = 0
        #rotate determined by initial orientation angle
        self.rd = self.Rot0[:2,:2].mv(self.rd.double()).float()


        self.rd_dot_tao = (self.rd-self.rd_prev)/dtao 
        self.rd_dot[0] = lambda_1*self.rd_dot_tao[0] - lambda_2*(0.1*(self.r[0]-self.rd[0]))
        self.rd_dot[1] = lambda_1*self.rd_dot_tao[1] - lambda_2*(0.1*(self.r[1]-self.rd[1]))
        self.rd_dot[0] = lambda_1*self.rd_dot_tao[0] - lambda_2*(0.1*math.sin(self.r[0]-self.rd[0]))
        self.rd_dot[1] = lambda_1*self.rd_dot_tao[1] - lambda_2*(0.1*math.sin(self.r[1]-self.rd[1]))
        self.rd_ddot[0] = 0
        self.rd_ddot[1] = 0

        #self.rd[0] = 0.6*self.t
        #self.rd[1] = 0
    ...
    def command_v_and_phi(self,rd_ddot,rd_dot,rd, r_dot, r,kp,kd ,  psi_cur , v_back,v_front, dt):
        ...
        v_dot = u[0]
        psi_dot = u[1]
        command_v_back = self.command_v_back+0.5*v_dot*dt
