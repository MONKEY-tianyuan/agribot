...
class CONTROLLER:
    ...
    rd = torch.zeros([2])
    rd_arr = torch.zeros([2,1886])
    rd_arr_adj = torch.zeros([2,1886])
    encoder1 = 0
    encoder3 = 0
    ...
    def __init__(self):
        ...
        R=6
        omega = 0.1/R
        for i in range(self.rd_arr.size()[1]):
            self.rd_arr[0,i] = R*math.sin(omega*i)
            self.rd_arr[1,i] = -R + R*math.cos(omega*i)
    ...
    def run_test(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.t<200:
                ...
                elif self.t_init>=5.0:
                    self.init_flag = 1
                    command_v_left = 0.9
                    command_v_right = command_v_left + self.dist_hori * self.omega[2]
                    self.command_v[0] = command_v_left
                    self.command_v[1] = command_v_right 
    ...
    def di2phi(self,di):
        ...
        if phi_left<-0.6:
            phi_left = -0.6
        if phi_left > 0.1 or phi_left<-0.1:
